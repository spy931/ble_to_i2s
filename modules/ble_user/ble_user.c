/**
 * @file ble_user.c
 * @brief BLE user application and service initialization.
 *
 * This file implements BLE stack, GAP, GATT, advertising, connection parameters,
 * and service initialization for the application, as well as BLE event handling
 * and accelerometer data updates.
 */
...
/**@brief   Macro for defining a ble_snd instance.
 *
 * @param   _name   Name of the instance.
 * @hideinitializer
 */
#define BLE_SND_DEF(_name)                                     \
  static ble_snd_t _name;                                      \
  NRF_SDH_BLE_OBSERVER(_name##_obs, BLE_LBS_BLE_OBSERVER_PRIO, \
                       ble_snd_on_ble_evt, &_name)

static ble_acc_t m_acc;
BLE_SND_DEF(m_lbs);       /**< LED Button Service instance. */
 ...
 *
 * Sets up all the necessary GAP (Generic Access Profile) parameters of the device,
...
void advertising_init(void) {
  ...
  ble_uuid_t adv_uuids[] = {{LBS_UUID_SOUND_SERVICE, m_lbs.uuid_type}};
 ...
 *
 * A pointer to this function will be passed to each service which may need to inform
...
void services_init(void) {
  ...
  // Initialize LBS.
  init.cmd_write_handler = snd_cmd_write_handler;

  err_code = ble_snd_init(&m_lbs, &init);
  APP_ERROR_CHECK(err_code);

  err_code = ble_acc_init(&m_acc);
}
 ...
 */
ret_code_t acc_data_update(uint16_t x_in, uint16_t y_in, uint16_t z_in) {
  ret_code_t err_code;
  err_code = ble_acc_on_data_change(m_conn_handle, &m_acc, x_in, y_in, z_in);
  NRF_LOG_INFO("BLE write exit code %d", err_code);
  return err_code;
}
